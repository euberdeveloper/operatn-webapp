<template lang="pug">
v-card
  v-card-title.headline.font-weight-regular.primary.white--text Inserimento contratto
  v-expand-transition
...
export default {
  ...
  methods: {
    ...
    deleteItem (item){
      confirm(`Sei sicuro di voler eliminare il contratto di ${item.nome} ${item.cognome} che inizierebbe il ${item.data_inizio} per la stanza ${item.numero_stanza} in ${item.descrizione_fabbricato} con contratto ${item.sigla}`) && this.$store.dispatch("inserimentoContratto/deleteItem", item)
    },
    signItem (item){
      confirm(`Confermi la firma del contratto di ${item.nome} ${item.cognome} che inizia il ${item.data_inizio} per la stanza ${item.numero_stanza} in ${item.descrizione_fabbricato} con contratto ${item.sigla}`) && this.$store.dispatch("inserimentoContratto/signItem", item)
    },
    closeItem (item){
      confirm(`Confermi la chiusura anticipata del contratto di ${item.nome} ${item.cognome} iniziato il il ${item.data_inizio} per la stanza ${item.numero_stanza} in ${item.descrizione_fabbricato} con contratto ${item.sigla}`) && this.$store.dispatch("inserimentoContratto/closeItem", item)
    },
    dismissDialog() {
