...
const documentDefinition = (firmatario, contratto) => {
	return {
		content: [
			{
				layout: 'noBorders',
				table: {
					...
					body: [
						[
							...
							[' ', ' ', ' ', {
								text: 'COMUNICAZIONE ASSEGNAZIONE FABBRICATO',
								alignment: "center",
								bold: true,
								fontSize: 14,
							}]
				...
				table: {
					...
					body: [
						[
							...
							[{
								text: 'Spettabile Questura di Trento',
								bold: true,
								fontSize: 12,
							},
								'Documento inviato per e-mail: urp.tn@poliziadistato.it']
						],
					]
				},
				margin: [0, 10, 0, 5]
			...
			`IL sottoscritto ${firmatario.nome} ${firmatario.cognome}, nato a ${firmatario.comune_nascita} il ${(new Date(firmatario.data_nascita)).toLocaleDateString('it-IT')} e residente a ${firmatario.residenza}, rappresentante legale dell'Opera Universitaria di Trento con sede in via Della Malpensada 82/A a Trento, comunica che in data ${(new Date(contratto.check_in)).toLocaleDateString('it-IT')} è stato assegnato per uso alloggio al/alla Signor/a :`,
			{
				table: {
					widths: ['*'],
					body: [
					...
					body: [
						[{
							...
							table: {
								body: [
									[{
										text: `${contratto.cognome.trim()} ${contratto.nome.trim()}`,
										bold: true,
									}],
									['cognome e nome']
								]
						...
						[{
							...
							table: {
								body: [
									[{
										text: `${contratto.data_nascita}`,
										bold: true,
									}, {
										text: `${contratto.comune_nascita}`,
										bold: true,
									}, {
										text: `${contratto.provincia_nascita}`,
										bold: true,
									}, {
										text: `${contratto.stato_nascita}`,
										bold: true,
									}, {
										text: `${contratto.cittadinanza}`,
										bold: true,
									}],
									['Data nascita', 'Comune di nascita', 'Provincia di nascita', 'Stato di nascita', 'Cittadinanza']
								]
							}
						}],
						[{
						...
						[{
							...
							table: {
								body: [
									[{
										text: `${contratto.tipo_documento}`,
										bold: true,
									}, {
										text: `${contratto.numero_documento}`,
										bold: true,
									}],
									['Tipo documento', 'Numero e data rilascio documento']
								]
							}
						}],
						[{
							...
							table: {
								body: [
									...
									['Autorita\' che ha rilasciato il documento']
								]
							}
						}],
					],
			...
			'Il fabbricato sottoindicato già adibito ad ABITAZIONE è sito in :',
			{
				layout: 'noBorders',
				table: {
					widths: ['*'],
					body: [
						[{
							...
							table: {
								body: [
									[{
										text: `${contratto.comune}`,
										bold: true,
									}, {
										text: `${contratto.provincia}`,
										bold: true,
									}, {
										text: `${contratto.cap}`,
										bold: true,
						...
						[{
							...
							table: {
								body: [
									[{
										text: `${contratto.indirizzo}`,
										bold: true,
									}],
									['Via o piazza, numero civico']
								]
							}
						}],
					],
			...
			},
			{
				...
				table: {
					...
					body: [
						[{
							text: `${contratto.interno || ''}`,
							bold: true,
						}, {
							text: `${contratto.descrizione_piano}`,
							bold: true,
						}, {
							text: `${contratto.numero_stanza}`,
							bold: true,
						}, {
							text: `${contratto.tipo_stanza}`,
							bold: true,
						}],
					]
				},
			...
			},
			{
				...
				table: {
					widths: [100, '*', 100, 100],
					body: [
						['Appartamento n.', 'Piano', 'Stanza n.', 'Singola/Doppia'],
					]
				},
				alignment: "center",
				margin: [0, 5, 0, 35]
...
export default (contratto) => {
	...
	Vue.prototype.$api.get(`/person/signer?date=${moment(contratto.check_in).format('YYYY-MM-DD')}`).then((res) => {
		pdfMake
			.createPdf(
				documentDefinition(
					res.data[0],
